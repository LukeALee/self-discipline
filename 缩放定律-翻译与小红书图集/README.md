...
`精读讲解视频/缩放定律精读讲解.mp4` —— 竖版 1080×1920、约 **14分56秒**、13 节的精读讲解视频(深色质感、关键词卡 + 推进式字幕 + 进度条)。

- **口播稿**:`精读讲解视频/缩放定律精读-口播稿.md`,逐字稿可直接照读录配音,字幕节奏与视频一致。
- **无人声**:本容器无可用的离线中文 TTS,故视频为「字幕讲解版」(无配音);配上口播稿即可自己一条条录音对上。
- 13 节内容经工作流逐节对照原文做了对抗式事实核查(已修正若干口径问题,如 Chinchilla「三种方法是否完全一致」、双重下降的出处归属等)。
- 重新生成:`node 精读讲解视频/render_video.js 输出.mp4`(读取同目录 `scenes.json`)。

> 小红书视频时长上限通常为 5 分钟,这条约 15 分钟更适合发 B 站/视频号,或在小红书剪成多段发布。
